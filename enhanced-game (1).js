...
function checkSavedGame() {
    const savedState = localStorage.getItem("gusulovestory_gameState");
    if (savedState) {
...
function continueGame() {
    ...
    if (savedState) {
        ...
        gameHistory = JSON.parse(localStorage.getItem("gusulovestory_gameHistory") || "[]");
        showGame();
        updateGame();
    }
}
...
// 保存游戏
function saveGame() {
    localStorage.setItem("gusulovestory_gameState", currentState);
    localStorage.setItem("gusulovestory_gameHistory", JSON.stringify(gameHistory));
    alert("游戏已保存！");
}
